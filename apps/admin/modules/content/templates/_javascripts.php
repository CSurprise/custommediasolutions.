<div class="sf_admin_form_row sf_admin_text sf_admin_form_field_javascripts">
	<div>
		...
		<div id="javascripts_container" class="content">
			...
			<?php foreach($_javascripts as $javascript):?>
				<?php echo $form['javascripts']->render(
					array(
					  'name' => 'content[javascripts][]',
					  'class' => 'javascript', 
					  'id' => 'content_javascripts_'.$i
					)
				);?>
				<script>
				$(function () {	
					$('#content_javascripts_<?php echo $i?>').val('<?php echo $javascript?>');
				});
				</script>
				<?php if( $i != 1 ):?>
				<a href="#" class="delete_content_javascript" rel="<?php echo $i;?>">Delete</a>
		...
		<div class="help add"><a href="#" id="add_content_javascript">Add Another</a></div>
