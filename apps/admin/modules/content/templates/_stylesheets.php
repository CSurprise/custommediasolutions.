<div class="sf_admin_form_row sf_admin_text sf_admin_form_field_stylesheets">
	<div>
		...
		<div id="stylesheets_container" class="content">
			...
			<?php foreach($_stylesheets as $stylesheet):?>
				<?php echo $form['stylesheets']->render(
					array(
					  'name' => 'content[stylesheets][]',
					  'class' => 'stylesheet', 
					  'id' => 'content_stylesheets_'.$i
					)
				);?>
				<script>
				$(function () {	
					$('#content_stylesheets_<?php echo $i?>').val('<?php echo $stylesheet?>');
				});
				</script>
				<?php if( $i != 1 ):?>
				<a href="#" class="delete_content_stylesheet" rel="<?php echo $i;?>">Delete</a>
		...
		<div class="help add"><a href="#" id="add_content_stylesheet">Add Another</a></div>
